...
// GeneratePDF generate a pdf file
func GeneratePDF() {
	m := GetMaroto()
	...
	err = document.Save("output/billingv2.pdf")
	if err != nil {
		log.Fatal(err.Error())
	}

	err = document.GetReport().Save("output/billingv2.txt")
	if err != nil {
		log.Fatal(err.Error())
	}
}

// GetMaroto return a Maroto struct pointer with basic configurations to generate a pdf

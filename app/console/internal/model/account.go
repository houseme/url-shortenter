...
type CreateAccountInput struct {
	*Base      `json:"-"`
	...
	GroupLevel uint   `json:"groupLevel" dc:"用户登记" v:"required|integer|in,0,1000,10000#用户等级 | 必须是整数 | 用户等级只能是 0,1000,10000"`
}
...
type ModifyPasswordInput struct {
	*Base    `json:"-"`
	Password string `json:"password" dc:"密码" v:"required|password2#请填写密码 | 密码需要 6-18 位，必须包含大小写字母和数字"`
	Code     string `json:"code" dc:"验证码" v:"required|between:4,6#请填写验证码 | 验证码长度为 4-6 位"`
}
